...
\begin{figure}[t]
    ...
    \caption[Hilltop quintessence mapping to $(w_0, w_a)$]{Inverse-variance weighted probability distribution $P(w_0,w_a)\propto 1/E_{H}^2$  showing the mapping from a hilltop quintessence model, Eq.~\eqref{eq:ch11-hilltop}, to the $(w_0, w_a)$ plane
...
\begin{figure}[!h]
    ...
    \caption[Training and validation loss for normalizing flows]{The training (black) and validation (\textcolor{red}{red}) loss for our $\Lambda$CDM parameter conditioned normalizing flow for hilltop ({\it top panel}) and exponential ({\it bottom panel}) priors. The \textcolor{yellow!80!black}{yellow} shaded region marks the interval after the minimum validation loss was reached before early stopping was triggered.}
...
\begin{figure}[ht!]
    ...
    \caption[Exponential quintessence mapping to $(w_0, w_a)$]{Same as Fig.~\ref{fig:ch11-1} for an exponential quintessence model, Eq.~\eqref{eq:ch11-exp}, with $\lambda=0.6$.}
    \label{fig:ch11-A1}
\end{figure}

\begin{figure*}[ht!]
    \centering    \includegraphics[width=0.95\linewidth]{figures/ch11/fig_App1_v0.pdf}
    \caption[Theory-informed priors for exponential quintessence]{Same as Fig.~\ref{fig:ch11-NF_priors} for an exponential quintessence model, Eq.~\eqref{eq:ch11-exp}. The normalizing flow (black lines) accurately reproduces the training data distributions, demonstrating robust learning across both quintessence model classes. As with the hilltop case, the mapped $(w_0, w_a)$ distributions are distinctly non-uniform despite originating from flat priors on $\lambda$. However, the exponential potential produces a broader peak around $(w_0, w_a) \approx (-1, 0)$, with the distribution extending more prominently toward smaller values of $w_0$ and more negative values of $w_a$, reflecting the fundamentally different thawing dynamics of exponential quintessence compared to hilltop models. }
    \label{fig:ch11-A2}
\end{figure*}
\clearpage
...
\begin{table*}[hb!]
	...
	\caption[Full parameter constraints under all priors]{Summary table of all cosmological parameter constraints from DESI DR2 BAO in combination with {\it Planck} and additional supernova datasets, under different priors, in the $w_0 w_a$CDM model. }
