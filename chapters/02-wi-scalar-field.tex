...
We assume that both constraints in Eqs.~\ref{eq:ch2-3.1}--\ref{eq:ch2-3.2} hold for the relevant period of $N\approx8$ $e$-foldings that can be probed in the CMB. In addition, we adopt a new time variable $x$ defined in Eq.~\ref{eq:ch2-3.4} in terms of the number $n$ of $e$-foldings since the beginning of the epoch, and in this new notation rewrite the overdamping constraint as Eq.~\ref{eq:ch2-3.5} and the density perturbation constraint as Eq.~\ref{eq:ch2-3.6}:
\begin{align}
...
\begin{align}
    f(x)&\equiv F(x)/\Bar{F}, \qquad h(x)\equiv H(x)/\Bar{H}, \nonumber\\
    q(x)&\equiv Q(x)/\Bar{Q}. \label{eq:ch2-3.9}
\end{align}
