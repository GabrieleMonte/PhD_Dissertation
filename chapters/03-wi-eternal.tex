...
tension with the slow-roll CI scenario.\footnote{For a detailed discussion on how WI is able to satisfy these different conditions, see e.g. Ref.\cite{Berera:2023liv}. }
...
The paper is organized as follows.  In Sec.~\ref{sec:ch3-EICI}, we briefly
...
Sec.~\ref{sec:ch3-EIWI}, we extend the discussion to the case of WI, where the
...
scalar power spectrum.  Our main results start from Sec.~\ref{sec:ch3-upper},
...
\begin{equation}
\frac{\delta\phi_Q}{\delta\phi_C} = \frac{H^2}{2 \pi \dot\phi} > 1.\label{eq:ch3-eternal}
\end{equation}
We note that the fraction (\ref{eq:ch3-eternal}) is identical to the amplitude of the curvature perturbation for modes crossing the horizon during inflation,
\begin{equation}
P\left(k\right) =\left(\frac{H^2}{2\pi\dot{\phi}}\right)^2. \label{eq:ch3-DeltaR2_CI}
\end{equation}
...
\begin{align}    &\delta\phi_D=\frac{|f(\phi)|}{H(\phi)},
  \label{eq:ch3-deltaphiD_C}\\
  &\delta\phi_S=\sqrt{\frac{2D^{(2)}(\phi)}{H(\phi)}}.
  \label{eq:ch3-deltaphiS_C}
\end{align}
%
...
\begin{eqnarray}
 ...
 \frac{|f(\phi)|}{H(\phi)}. \label{eq:ch3-FDR}
\end{eqnarray}
%
...
\end{eqnarray}
%
which matches the amplitude of the scalar curvature perturbations, Eq.~\ref{eq:ch3-DeltaR2_CI}. Hence, again we find the result of Eq.~\ref{eq:ch3-FDR_CI} above, that the condition for EI in CI is that the curvature
...
Eq.~\ref{eq:ch3-FDR_CI}.  We explore this statement explicitly in the next section.
...
%
\begin{eqnarray}
  \dot{\phi}=\frac{1}{3H+\Upsilon}\left(-V_{,\phi}(\phi)+\xi_q+\xi_T\right), \label{eq:ch3-EOM_warm}
\end{eqnarray}
...
in the denominator we normalize the scale factor $a$, such that $a(t=1/H)=1$.  With these substitutions, Eq.~\ref{eq:ch3-EOM_warm} becomes:
%
\begin{align}
    \dot{\phi}&=-f_w(\phi)+\sqrt{2D^{(2)}_{\rm{vac}}}\,\zeta_q
    +\sqrt{2D^{(2)}_{\rm{diss}}}\,\zeta_T,
\label{eq:ch3-WIlangevin}
\end{align}
%
...
T\}$.  In Eq.~(\ref{eq:ch3-WIlangevin}), we also have that $f_w(\phi)$ is the
...
%
\begin{align}
  &\delta\phi^{w}_D=\frac{|f_w(\phi)|}{H},\label{eq:ch3-deltaphiD_W}
  \\
  &\delta\phi^{w}_S=\sqrt{\frac{2\left(D^{(2)}_{\rm{vac}}+D^{(2)}_{\rm{diss}}
      \right)}{H}}. \label{eq:ch3-deltaphiS_W}
\end{align}

Combining Eqs.~\ref{eq:ch3-driftC_warm}--\ref{eq:ch3-deltaphiS_W}, the FDR
condition, Eq.~\ref{eq:ch3-FDR1}, in the WI context can now be expressed
as
%
\begin{eqnarray}
    \frac{H}{2\pi
      \dot{\phi}}>\frac{1+Q}{\sqrt{(1+2n_*)\sqrt{1+\frac{4\pi
            Q}{3}}+2\pi Q \,\frac{T}{H}\, }}. \label{eq:ch3-FDR_warmv0}
\end{eqnarray}
...
direct coupling of the inflaton and radiation fluctuations due to a temperature-dependent dissipation rate, and can only be determined numerically by solving the full set of perturbation equations~\cite{Graham:2009bf, Bastero-Gil:2011rva,Montefalcone:2023pvh}. For a dissipation rate $\Upsilon\propto T^c$, if $c=0$ we have $G(Q)=1$, while $G(Q)$ is larger (smaller) than one for $c >0$ ($c<0$).
 ...
 We can now use Eq.~\ref{eq:ch3-DeltaR_warm} and rewrite the FDR condition in WI, Eq.~\ref{eq:ch3-FDR_warmv0}, in terms of the amplitude of the curvature perturbations, similarly to what we previously did for the CI case. In formulas, the condition for EI in WI is
%
\begin{eqnarray}
    P(k)> P_{\rm{FDR}} \,\,\,\,\,\,\,   ({\rm
 WI}) \, ,\label{eq:ch3-EIC_warm}
\end{eqnarray}
where
\begin{eqnarray}
    P_{\rm{FDR}}\equiv (1+Q)^2\frac{\left(1 + 2 n_{*}+\frac{2 \sqrt{3}
        \pi Q}{\sqrt{3+4 \pi Q}}
      \frac{T}{H}\right)G(Q)}{(1+2n_*)\sqrt{1+\frac{4\pi Q}{3}}+2\pi Q\,\frac{T}{H}\, }. \label{eq:ch3-P_FDR}
\end{eqnarray}
...
Eq.~\ref{eq:ch3-EIC_warm} reduces to the standard CI result: $P(k)>1$. For $Q>0$, we have that $P_{\rm{FDR}}>1$. More specifically, there
...
Eq.~\ref{eq:ch3-EIC_warm}, we obtain that
%
\begin{equation}
...
Eq.~\ref{eq:ch3-EIC_warm} clearly shows that in a WI setting, the onset of EI is suppressed relative to the standard CI case, and
...
$P_{\rm{FDR}}$, Eq.~\ref{eq:ch3-P_FDR}. } at a rate proportional to the
...
0.0067. \label{eq:ch3-Planck_alphaS}
     \end{align}
%
...
condition for EI, Eq.~\ref{eq:ch3-EIC_warm}, is satisfied.  Thus, as
...
\begin{eqnarray}
    ...
    \label{eq:ch3-alpha_WI_bound_gen}
\end{eqnarray}
%
...
%
\begin{equation}
    \alpha_s< -1.3 \times 10^{-4} \,\,\,\,\,\,\,\,\,\, {\rm (CI)}. \label{eq:ch3-alpha_CI_bound}
\end{equation}
%
The bound on the running, generalized in the WI context, is therefore
\begin{equation}
    \alpha_s<- 1.3\times 10^{-4} \times \frac{\ln P_{*}}{\ln(P_{*}/P_{\rm{FDR}})}\,\,\,\,\,\,\,\,\,\, {\rm (WI)}, \label{eq:ch3-alpha_WI_bound}
\end{equation}
where $\ln
...
for the condition of Eq.~\ref{eq:ch3-alpha_WI_bound} are chosen when the
FDR condition in Eq.~\ref{eq:ch3-FDR} is an equality, i.e., at the
...
\begin{equation}
    \alpha_s< -4.4 \times 10^{-5}, \qquad \text{$(Q=100, b_G=6.52)$}\label{eq:ch3-alpha_WI_MWI_bound}
\end{equation}
...
to non-Abelian gauge fields. MWI is realized in the SDWI regime ($Q\gg1$) and for a dissipation rate cubic in temperature, i.e. $\Upsilon\propto T^3$; hence the upper bound on the running in Eq.~\ref{eq:ch3-alpha_WI_MWI_bound} roughly applies. The running of the
spectral index associated with this model was recently computed in~\cite{Das:2022ubr}, with the result
\begin{equation}
\alpha_s^{\rm{MWI}}\simeq -2\times 10^{-4}, \label{eq:ch3-alpha_MWI_computed}
\end{equation}
which, according to the bound in Eq.~\ref{eq:ch3-alpha_WI_MWI_bound}, is
...
Eq.~\ref{eq:ch3-alpha_WI_MWI_bound}.
...
Eq.~\ref{eq:ch3-Planck_alphaS}, at the 1$\sigma$ level. This emphasizes another relevant takeaway of our work: within the context of WI, CMB data are consistent with a large
...
\end{equation}
the bound in
Eq.~\ref{eq:ch3-alpha_WI_bound} to prevent eternal inflation
becomes:
\begin{equation}
...
\end{equation}
%
which is always satisfied as long as Eq.~\ref{eq:ch3-alpha_WI_bound}
holds and
\begin{equation}
...
\begin{eqnarray}
    \alpha_s<- 1.3\times 10^{-4} \times \frac{\ln P_{*}}{
      \ln(P_{*}/P_{\rm{FDR}})}, \label{eq:ch3-Conclusion_1}
\end{eqnarray}
where $P_*$ is the amplitude of the scalar power spectrum (at the
relevant scale $k_*$ for CMB observations) and $P_{\rm{FDR}}$, defined in Eq.~\ref{eq:ch3-P_FDR}, represents the upper limit on the scalar power spectrum beyond which a
...
(see for instance Eq.~\ref{eq:ch3-EIC_SDwarm}). This means that in a WI setting, an even tinier amount of
...
Eq.~\ref{eq:ch3-Conclusion_1} i.e., they do not have EI. These include the Minimal Warm Inflation (MWI)~\cite{Berghaus:2019whh} model and its variants~\cite{Das:2020xmh} in the strongly dissipative regime ($Q\gg 1$)~\cite{Das:2022ubr}; as well as a quartic or a
...
be achieved as long as Eq.~\ref{eq:ch3-Conclusion_1} holds and
